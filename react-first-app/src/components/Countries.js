    ...
    return (
        <div className='countries'>
            <ul className="radio-container">
                ...
                {radio.map((continent) => (
                    <li>
                        <input type="radio" id={continent} name="continentRadio" checked={continent === selectedRadio} onChange={(e) => setSelectedValue(e.target.id)} />
                        <label htmlFor={continent}>{continent}</label>
                    </li>
                ))}
            </ul>
            {selectedRadio && (
                <button onClick={() => setSelectedValue("")}>Annuler les filtres</button>
            )}
            <ul>
                {data
